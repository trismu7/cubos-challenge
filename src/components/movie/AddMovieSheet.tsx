...
import {
  createMovieAction,
  fetchImdbDataAction,
} from "@/lib/actions/movie/create/createMovie";
import { processImdbResponse } from "@/lib/utils";

export default function AddMovieSheet({
  onAddMovie,
}: {
  onAddMovie: () => void;
}) {
  ...
  const watchedReleaseDate = form.watch("releaseDate");
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        Object.entries(processedData).forEach(([key, value]) => {
          if (value !== undefined) {
            form.setValue(key as keyof AddMovieFormData, value as never);
          }
        });
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button className="grow">Adicionar filme</Button>
      </SheetTrigger>
